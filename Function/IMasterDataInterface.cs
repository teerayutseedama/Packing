    ...
    public class MasterDataInterface: IMasterDataInterface
    {
        ...
        public async Task<MasterDataView> GetMasterDataView()
        {
           var result= new MasterDataView();
            try
            {
                result.LineList = await _context.tbm_pk_production_line.Select(x => new LineList { PK_LINE_NAME = x.PK_LINE_NAME, PACKING_LINE_ID = x.PACKING_LINE_ID }).ToListAsync();
                ...
                result.MaterialList = await _context.tbm_material.Select(x => new MaterialList { MATERIAL_CODE = x.MATERIAL_CODE, MATERIAL_NAME = x.MATERIAL_NAME }).ToListAsync();
            }
            catch (Exception ex)
            {
