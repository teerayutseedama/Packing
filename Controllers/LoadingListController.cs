...
namespace Packing.Controllers
{
    public class LoadingListController : Controller
    {
        ...
        public LoadingListController(ILoaddingListInterface loadding)
        {
            _loadding = loadding;
        }
        ...
        int row = 2;
                foreach (var item in dataList)
                {
                    ...
                    worksheet.Cell(row, 9).Value = item.Qty;
                    worksheet.Cell(row, 10).Value = item.UOM;
                    row++;
                }
